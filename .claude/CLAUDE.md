...
Using a coding agent other than Claude Code? `AGENTS.md` at the repo root maps this contract onto tools without hooks or slash commands. This file remains canonical.

## Non-Negotiables

1. **Preamble first.** The SessionStart hook auto-prints a digest (config, MCP state, git, **remote visibility**, staleness flags, P0 count) — act on what it flags, then run the full preamble (spec: `frameworks/preamble.md`, or `/preamble`). Mint session IDs with `bash hooks/next-session-id.sh` — never count events; counting collides as soon as one start event is lost or duplicated.
...
5. **Quality-gate everything outbound.** Writing rules below; `hooks/check-quality.sh` validates, and a PreToolUse hook hard-blocks email drafts that fail. Drafts only — never auto-send. Never route around the gate via another channel.
...
9. **This repo stays private.** It accumulates client data by design. If the digest's visibility probe warns the origin is public, fixing that outranks every other task in the session.

## First Run

A fresh instance has template knowledge files and an `unconfigured` MCP seed — the digest will say so. Run **`/bootstrap`**: a guided, resumable setup that interviews the user, researches their market and product, writes the knowledge files, fills this file's routing-table tool slots, probes MCPs into `memory/mcp-status.json`, and creates first dossiers. **Joining a team that already runs this system:** shared knowledge files imported before `/bootstrap` switch phases 2–3 from generate to import-and-localize (`docs/start-here/team-adoption.md`). Manual path: `docs/start-here/customize-for-your-domain.md`.

## Who I Am

Loaded from `.claude/MY-CONFIG.md` at session start: identity, territory, account list, service IDs. If missing, run `/bootstrap` (or `cp .claude/MY-CONFIG.template.md .claude/MY-CONFIG.md` and validate with `bash hooks/check-config.sh`).
...
**Start:** run the preamble (`frameworks/preamble.md` or `/preamble`) — config check, `git pull` via `hooks/git-safe.sh`, load active-context + TODOS + last 20 handoff lines, re-probe MCPs if `memory/mcp-status.json` is stale (>24h) or still the `unconfigured` seed, staleness scan, mint ID + log `session_start`, announce priorities.

**End:** on **"end session"**, **"wrap up"**, **"done for today"**, or `/end-session` — roll active-context forward (3-session lookback; older content archived), update action items, log `session_end` (same session_id), append a handoff line, commit + push. Every session ends with context saved and pushed.
...
**Already automated — don't re-do manually:** SessionStart digest (`hooks/session-start.sh` — includes the public-remote privacy probe and first-run `/bootstrap` hint), unclean-termination marker (`hooks/session-end.sh`), outbound draft quality gate (`hooks/check-outbound-quality.sh`) — wired in `.claude/settings.json`.
...
Read the listed files BEFORE producing. Log `framework_used` after. Bracketed tool slots (`[your CRM MCP]`, …) are filled by `/bootstrap` phase 5 — slot reference and degraded-mode guidance: `docs/start-here/connect-your-stack.md`. A slot marked `— (manual)` means do that task's data step by hand; don't silently skip the task.
...
| Outbound sequences (incl. sourcing contacts into flows, launch hygiene) | `frameworks/sequences.md` + `knowledge/communication-playbook.md` + `knowledge/domain-summary.md` |
| Event registration drive (hosted webinar/event inside 14 days) | `frameworks/sequences.md` → "Event Registration Drive" + `knowledge/communication-playbook.md` |
...
| New / restructured account file | `frameworks/account-dossier.md` (worked example: `accounts/EXAMPLE-northwind-insurance.md`, fictional) |
...
**Knowledge depth:** default `knowledge/domain-summary.md` (1-pager); deep files only for business cases, competitive reviews, or new-account onboarding. Knowledge files are source of truth — augment with fresh research, never contradict; if research contradicts a file, correct the file. Full knowledge model: `docs/reference/knowledge.md`.
...
- Use only proof points and data-scale claims that exist in YOUR `knowledge/domain-summary.md` with sources — never invent metrics, client names, or case-study specifics.
...
- **Parallelize independent fetches** in one turn (call prep: calendar + analytics + internal search simultaneously).
...
- **MCP health is probed, not assumed:** `memory/mcp-status.json` carries per-MCP probe tools/args; `hooks/check-mcp.sh` fails on broken or stale (>24h) and reports "setup pending" on a fresh instance. Tasks depending on a broken MCP fail silently — announce BROKEN immediately.

## Reference Index (lazy-load — read when the topic comes up)
...
| First-run setup / instance build | `.claude/commands/bootstrap.md` · `docs/start-here/quick-start.md` |
| Team onboarding (shared knowledge import) | `docs/start-here/team-adoption.md` |
| Tool slots / MCP setup / degraded modes | `docs/start-here/connect-your-stack.md` |
| The full loop, worked examples (fictional) | `docs/start-here/example-session.md` · `accounts/EXAMPLE-northwind-insurance.md` |
| New-user questions + glossary | `docs/start-here/faq.md` |
...
| Knowledge model (what to build, what ships) | `docs/reference/knowledge.md` + `knowledge/README.md` |
| Your vertical (build these via `/bootstrap`) | `knowledge/domain-summary.md` · `knowledge/product-capabilities.md` · `knowledge/communication-playbook.md` |
| Cross-tool agents (non-Claude) | `AGENTS.md` |
| Staying current with upstream | `docs/start-here/fork-guide.md` → "Staying current" |
