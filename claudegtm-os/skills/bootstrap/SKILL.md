---
name: bootstrap
description: Guided first-run setup — interview + research, then write your knowledge files, wire tool routing, probe MCPs, create first dossiers. Triggers: "bootstrap", "set up claudegtm", "first run".
---
...
3. **Team-adoption check.** Ask once: "Did a teammate share knowledge files with you?" (the team path: `docs/start-here/team-adoption.md`). If yes — or if Phase-0 inventory found non-template knowledge files the user didn't write — Phases 2 and 3 switch from *generate* to **import-and-localize**: validate the shared files (structure, no placeholder text, one-page limit), then ask what differs about THIS user's patch (territory, segment, named accounts, regulatory flavor) and adjust the files accordingly. Flag anything in the shared files that research contradicts. Phases 1, 4, 5, 6 always run in full — identity, voice, tool wiring, and dossiers are never transferable.
